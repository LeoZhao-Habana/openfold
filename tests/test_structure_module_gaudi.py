...
class TestStructureModule(unittest.TestCase):
    def test_structure_module_shape(self):
        ...
        s = torch.rand((n, c_s)).to(device)
        z = torch.rand((n, n, c_z)).to(device)
        f = torch.randint(low=0, high=21, size=(n,)).long().to(device)
        ...
        self.assertTrue(out["frames"].shape == (no_layers, n, 7))
        self.assertTrue(
            out["angles"].shape == (no_layers, n, no_angles, 2)
        )
        self.assertTrue(
            out["positions"].shape == (no_layers, n, 14, 3)
        )
